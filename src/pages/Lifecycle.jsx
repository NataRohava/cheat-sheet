...
export const LifeCycle = () => {
  return (
    <div>
      ...
      <p>
        <b>Жизненный цикл (lifeCycle) компонента</b> - это шаги (этапы), которые
        проходят компоненты:
        <span style={{ backgroundColor: "#C0C0C0" }}>
          создаются,обновляются,удаляются
        </span>{" "}
      ...
      <ol>
        <li>
          ...
          <ul>
            <li>
              <b style={{ color: "#A52A2A" }}>constructor()</b>
            </li>
            <li>
              <b style={{ color: "#A52A2A" }}>render()</b>
            </li>
            <li>
              <b style={{ color: "#A52A2A" }}>componentDidMount()</b>
            </li>
          </ul>
        </li>
        <li>
          ...
          <ul>
            <li>
              <b style={{ color: "#A52A2A" }}>shouldComponentUpdate()</b>
            </li>
            <li>
              <b style={{ color: "#A52A2A" }}>render()</b>
            </li>
            <li>
              <b style={{ color: "#A52A2A" }}>componentDidUpdate()</b>
            </li>
          </ul>
        </li>
        <li>
          ...
          <ul>
            <li>
              <b style={{ color: "#A52A2A" }}>componentWillUnmount()</b>
            </li>
          </ul>
        </li>
        <li>
          ...
          <ul>
            <li>
              <b style={{ color: "#A52A2A" }}>componentDidCatch()</b>
            </li>
          </ul>
        </li>
      </ol>
      <p>
        ...
        <p>
          ...
          компонентах предназначек хук{" "}
          <b style={{ color: "#A52A2A" }}>useEffect</b> .
        </p>
        <p>
          Первый параметр:{" "}
          <span style={{ backgroundColor: "#C0C0C0" }}>функция</span> , которая
      ...
      <p>
        ...
        <br />
        Заменяет{" "}
        <span style={{ backgroundColor: "#C0C0C0" }}>componentDidMount()</span>.
        Выполняет побочные эффекты только один раз после монтирования{" "}
        <span style={{ fontStyle: "italic" }}>
          (запуститься один раз при первом рендере)
        </span>{" "}
        .
      </p>
...
`}
      ...
      <p>
        <b style={{ font: "bold" }}>В массиве указаны значения.</b>
        <br />
        <b
          style={{ color: "#A52A2A" }}
        >{`useEffect (() => {}, [dependency])`}</b>
        <br />
        Заменяет{" "}
        <span style={{ backgroundColor: "#C0C0C0" }}>componentDidUpdate()</span>
        . Позволяет указать зависимость для выполнения функции при изменении
        определенных props или state{" "}
        <span style={{ fontStyle: "italic" }}>
          (запуститься при изменении dependency)
        </span>
...
`}
      ...
      <p>
        <b style={{ font: "bold" }}>Нет массива.</b>
        <br />
        <b style={{ color: "#A52A2A" }}>{`useEffect (() => {})`}</b>
        <br />
        Отрабатывает как{" "}
        ...
        <span style={{ fontStyle: "italic" }}>
          (запускается после каждого рендеринга компонента)
        </span>{" "}
        .
      </p>
      <SyntaxHighlighter language="javascript" style={a11yLight}>
...
`}
      ...
      <ul>
        <li>
          <b style={{ color: "#A52A2A" }}>constructor()</b>: вызывается при
          создании экземпляра компонента для инициализации состояния и привязки
          методов.
        </li>
        <li>
          <b style={{ color: "#A52A2A" }}>render()</b>: отрисовывает компонент и
          возвращает React элементы.
        </li>
        <li>
          <b style={{ color: "#A52A2A" }}>componentDidMount()</b>: вызывается
          после рендеринга компонента для выполнения запросов к серверу,
          подписки и др.
        </li>
      </ul>
      <SyntaxHighlighter language="javascript" style={a11yLight}>
...
`}
      ...
      <ul>
        <li>
          ...
          : вызывается перед обновлением компонента для оптимизации перерисовки
        ...
        <li>
          ...
          : вызывается после обновления компонента и его рендеринга для
          выполнения дополнительных действий после обновления компонента (для
...
`}
      ...
      <ul>
        <li>
          <b style={{ color: "#A52A2A" }}>componentWillUnmount()</b>: Вызывается
          перед удалением компонента из DOM для выполнения необходимых очисток.
        </li>
      </ul>
      <SyntaxHighlighter language="javascript" style={a11yLight}>
...
`}
      ...
      <ul>
        <li>
          <b style={{ color: "#A52A2A" }}>componentDidCatch(error, info)</b>:
          вызывается при возникновении ошибки в дочерних компонентах для
          отлавливания и обработки ошибок.
        </li>
      </ul>
      <SyntaxHighlighter language="javascript" style={a11yLight}>
...
`}
      ...
      <h3>
        <Link to="/didupdate">
          Пример useEffect как замена componentDidUpdate
        </Link>
      </h3>
    </div>
  );
