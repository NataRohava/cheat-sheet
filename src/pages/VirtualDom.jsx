export const VirtualDom = () => {
  return (
    <div>
      <h2>VirtualDOM</h2>
      <p>
        <b>DOM</b> - это сокращение от Document Object Model (объектная модель
        документа). Когда ты открываешь веб-страницу в браузере, браузер создает
        структуру документа, которую можно представить в виде дерева элементов.
        Эти элементы - это HTML-теги. DOM позволяет нам манипулировать этой
        структурой, изменять содержимое, стили, добавлять и удалять элементы.
      </p>
      <p>
        Каждый раз, когда происходит изменение данных в приложении, React должен
        обновить DOM, чтобы отобразить эти изменения на странице. Но вместо
        того, чтобы непосредственно вносить изменения в реальный DOM, React
        использует виртуальный DOM.
      </p>
      <p>
        <b>Виртуальный DOM</b>-это промежуточное представление DOM в памяти
        ...
        <b>сравнивает это виртуальное дерево с реальным DOM </b>и определяет,
      ...
      <ol>
        ...
        <li>
          <b>Переносимость:</b> Ты можешь писать свое приложение, не думая о
          том, какой браузер будет его запускать. React самостоятельно
          обрабатывает эти различия.
        </li>
      </ol>
      <p>
        Использование виртуального DOM позволяет React быть{" "}
        <b>эффективной, удобной</b> в использовании библиотекой.
      </p>
      <h3>Reconcilation</h3>
      <p>
        <b>Reconciliation</b> - это процесс, при котором React{" "}
        <b>сравнивает предыдущее состояние компонента с новым состоянием</b> и
        определяет, какие изменения нужно внести в пользовательский интерфейс.
      </p>
      ...
      <ol>
        ...
        <li>
          <b>Сравнение элементов:</b>React сравнивает элементы, которые
      ...
      <p>
        Таким образом, при взаимодействии с компонентом Counter, React обновляет
        только те части интерфейса, которые реально нуждаются в изменениях. Это
        делает процесс обновления более эффективным и позволяет приложению
